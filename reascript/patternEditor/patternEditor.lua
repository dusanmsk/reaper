...
-- return true if there is free space on selected block of track
pattern.isEmpty = function(trackNo, lineStart, lineEnd)
    -- todo implement
    for i = lineStart, lineEnd, 1 do
        if pattern.getRecord(i, trackNo) ~= nil then return false end
    end
    return true
end

-- return track number where specified block is empty or nil if there is no such block
pattern.getFreeTrack = function(lineStart, lineEnd)
    for trackNo = 0, gui.numOfTracks - 1, 1 do
        if pattern.isEmpty(trackNo, lineStart, lineEnd) then return trackNo end
    end
    return nil
end
...
function importUnknownMidiClip()
    ...
    if retval then
        ...
        -- read notes by channel (should be midi clip generated by pattern editor but with lost sysex id
        retval, notecnt, ccevtcnt, textsyxevtcnt = reaper.MIDI_CountEvts(pattern.take)
        for noteIdx = 0, notecnt - 1, 1 do
            ...
            rec.pitch = pitch
            rec.velocity = vel
            if not pattern.isEmpty(trackNo, posStart, posStart + noteLen) then
                trackNo = pattern.getFreeTrack(posStart, posStart + noteLen)
                assert(trackNo ~= nil, "Can't insert note into pattern, no free tracks")
            end
            -- insert note
